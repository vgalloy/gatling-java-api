...
public final class WsSimulation extends SimulationWrapper {
    ...
    protected void configure() {
        ...
        ScenarioBuilderWrapper scn = scenario("WebSocket")
                ...
                .pause(1, TimeUnit.MICROSECONDS)
                .repeat(2, "i",
                        exec(ws("Say Hello WS").sendText("{\"text\": \"Hello, I'm ${id} and this is message ${i}!\"}"))
                                .pause(1, TimeUnit.MICROSECONDS)
                ).exec(ws("Message1")
                        .sendText("{\"text\": \"Hello, I'm ${id} and this is message ${i}!\"}")
//                        .check(Predef.wsAwait().within(30, TimeUnit.SECONDS).expect(1).jsonPath("$.message").findAll.saveAs("message1"))
                )
                .exec(ws("Message2")
                        .sendText("{\"text\": \"Hello, I'm ${id} and this is message ${i}!\"}")
//                        .check(wsListen.within(30seconds).until(1).jsonPath("$.message").saveAs("message2"))
                ).exec(ws("Message3")
                        .sendText("{\"text\": \"Hello, I'm ${id} and this is message ${i}!\"}")
//                        .check(Predef.wsAwait().within(30seconds).expect(1).regex("$.message").saveAs("message3"))
                ).exec(ws("Message3")
                        .sendText("{\"text\": \"Hello, I'm ${id} and this is message ${i}!\"}")
//                        .check(wsListen.within(30seconds).expect(1).message)
                );
//                .exec(ws("Message3")
//                        .check(wsListen.within(30seconds).expect(1).message)
//                ).exec(ws("Cancel")
...
//                ).exec(ws("Message4")
//                        .sendText("{\"text\": \"Hello, I'm ${id} and this is message ${i}!\"}")
//                        .check(Predef.wsAwait().within(30seconds).until(1))
//                );
