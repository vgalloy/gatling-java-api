 ...
 */
public final class WsSendTextFrameBuilderWrapper implements Supplier<WsSendTextFrameBuilder>, ActionBuilderSupplier {
    ...
    public WsSendTextFrameBuilderWrapper await(long length, TimeUnit unit, final WsTextFrameCheck... wsTextFrameCheck) {
        final FiniteDuration finiteDuration = FiniteDuration.apply(length, unit);
        final WsSendTextFrameBuilder newInstance = get().await(finiteDuration, ScalaHelper.map(wsTextFrameCheck));
